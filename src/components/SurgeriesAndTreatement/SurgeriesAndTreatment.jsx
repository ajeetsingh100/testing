...
import surgeriesAndtreatment from '../../Data/surgeryAndTreatment'
...
const SurgeriesAndTreatment = () => {
  ...
  useEffect(() => {
    setLoading(true)
    const sideMenuData=surgeriesAndtreatment.filter(data=>data.navTo===searchedKeyword1)
    ...
    <div className='w-100  d-flex flex-column align-items-center justify-content-center'>
      <div className=' position-relative d-flex justify-content-center  mb-4 w-100 ' style={{height:'200px'}}>
        ...
        <div
          ...
          <div className="container position-absolute d-flex justify-content-center align-items-center text-white w-100 h-100">
              <div className=' rounded-2 p-3' style={{backgroundColor:"rgba(182, 176, 176, 0.5)"}}>
                    Home  | Surgeries and Treatement | <span className='text-warning'> {changeCase({searchedKeyword2})}</span>
              </div>
            </div>
                ...
                <table className=' table text-start table-bordered  table-hover'>
                  <tbody>
                  {
                    sidebarData[0]?.links.map(link=>
                      <tr className=''>
                       <td className=' bg-light' ><a href={`/surgeries&treatments/${searchedKeyword1}/${link.hyperLink}`} className='text-decoration-none' style={{color:"#7A0404"}}><div>{link.linkName} </div></a></td>
                      </tr>
                    )
                  }
                  </tbody>
